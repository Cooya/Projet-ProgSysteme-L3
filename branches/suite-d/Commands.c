...
int execExternalCommand(char **args)
{
    ...
    else
    {
        ...
        //return job_wait( job_current() , bg);
    }
    return 0;
...
int cmdCd(char **args) { // manque les chemins relatifs
	char* dest;
	if (args[1]==NULL)
		dest = getEnvVar("HOME");
	else
...
int cmdPushd(char **args) { // manque les chemin relatifs
	if(args[1]==NULL) {
		...
		char* path = mallocPathForStack(args[1]);
		dllPushFront(directoriesStack, &path);
		printDirectoriesStack();
	}
	return 0;
...
{
    //fprintf(stdout, "cmdExec args[0] = %s \n", args[1]);
    if(externalCommandExists(args[1]))
        return execExternalCommand(args+1);
    else
